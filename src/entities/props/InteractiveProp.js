// src/entities/props/InteractiveProp.js
...
export class InteractiveProp extends BaseEntity {
  ...
  onInit(data) {
    ...
    this.useCount     = 0;
    this.maxUses      = Number(data.maxUses  ?? -1);  // -1 = infini
    this.action       = String(data.action   || "none"); // none | toggle | trigger | give
  ...
  interact(playerId) {
    ...
    if (now - this.#lastUsed < this.cooldownMs) {
      return { ok: false, reason: "cooldown", remaining: this.cooldownMs - (now - this.#lastUsed) };
    }
