...
export class PhysicsProp extends BaseEntity {
  ...
  onInit(data) {
    this.mass        = Number(data.mass        ?? 1);
    this.friction    = Number(data.friction    ?? 0.5);
    this.restitution = Number(data.restitution ?? 0.3);
  ...
  toJSON() {
    return { ...super.toJSON(), mass: this.mass, friction: this.friction, restitution: this.restitution, isKinematic: this.isKinematic, velocity: this.velocity, angularVel: this.angularVel, sleeping: this.sleeping };
  }
}
